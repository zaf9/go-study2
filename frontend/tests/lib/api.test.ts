...
describe("api 拦截器行为", () => {
  ...
  it("业务错误时抛出异常", async () => {
    const handler = (api as any).interceptors.response.handlers[0].fulfilled;
    await expect(handler({ data: { code: 40004, message: "bad" } })).rejects.toThrow("bad");
  });
  ...
  it("401 时尝试刷新并重试原请求", async () => {
    ...
    (api as any).request = jest.fn().mockResolvedValue("retried");
    const error = {
      response: { status: 401 },
      config: { headers: {} },
    };
    ...
    expect((api as any).request).toHaveBeenCalled();
