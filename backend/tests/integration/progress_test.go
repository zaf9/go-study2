...
func TestProgressFlow_EndToEnd(t *testing.T) {
	...
	if adminData.NeedPasswordChange {
		changeReqBody := fmt.Sprintf(`{"oldPassword":"%s","newPassword":"ProgressFlow123!"}`, adminPassword)
	...
	registerReq, _ := http.NewRequest(http.MethodPost, baseURL+"/api/v1/auth/register", bytes.NewBufferString(`{"username":"progress_flow","password":"TestPass123!","remember":true}`))
	...
	userLogin := doIntegrationPost(t, client, baseURL+"/api/v1/auth/login", `{"username":"progress_flow","password":"TestPass123!","remember":true}`)
