...
func TestRegisterRoutes(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		resp2, err := client.Get(nil, "/api/v1/topic/lexical_elements?format=json")
		t.AssertNil(err)
		defer resp2.Close()
		t.Assert(resp2.StatusCode, 200)

		// 测试 lexical_elements 内容路由
		resp3, err := client.Get(nil, "/api/v1/topic/lexical_elements/comments?format=json")
		t.AssertNil(err)
		defer resp3.Close()
		t.Assert(resp3.StatusCode, 200)

		// 测试 constants 菜单路由
		resp4, err := client.Get(nil, "/api/v1/topic/constants?format=json")
		t.AssertNil(err)
		defer resp4.Close()
		t.Assert(resp4.StatusCode, 200)

		// 测试 constants 内容路由
		resp5, err := client.Get(nil, "/api/v1/topic/constants/boolean?format=json")
		t.AssertNil(err)
		defer resp5.Close()
...
func TestRegisterRoutesWithMiddleware(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		// 测试格式中间件：无效格式
		resp, err := client.Get(nil, "/api/v1/topics?format=invalid")
		t.AssertNil(err)
		defer resp.Close()
