...
## 构建与检查
- 在仓库根执行：`cd backend && go test ./... && go build ./...`（如有 `./build.bat` 可优先执行）
- 静态检查：`powershell -ExecutionPolicy Bypass -File backend/scripts/check-go.ps1`
- 覆盖率：`cd backend && go test ./... -cover`

## CLI 模式
- 启动：`cd backend && go run main.go`，主菜单选择 `2` 进入 Variables
- 子菜单 0-3 覆盖 `storage|static|dynamic|zero`，展示内容和测验（含答案解析）

## HTTP 模式
- 启动：`cd backend && go run main.go -d`（默认 127.0.0.1:8080）
- API：
