...
- [x] **v0.7** - 学习进度与测验体验优化 🆕
  - [x] 准确的章节数量显示（从static-routes.ts获取真实数据）
  - [x] 完整的章节学习状态（未开始/学习中/已完成三种状态）
  - [x] 可用的章节测验功能（修复加载失败和提交错误）
  - [x] 统一的进度数据展示（所有页面数据源一致，SWR缓存管理）
  - [x] 便捷的测验导航体验（测验中心页面，≤2步访问路径）
  - [x] 错误处理与边缘情况（空数据引导、并发提交保护、localStorage暂存）
  - [x] 性能优化（数据库索引、SWR缓存策略、React.memo组件优化）

### 进行中 🚧

- [ ] **v0.8** - 文档完善
  - [x] README.md更新（feature 016）
  - [ ] API文档完善
  - [ ] 贡献指南
  - [ ] 使用教程视频
