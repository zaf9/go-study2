...
3) 登录后访问注册接口（携带 JWT）尝试创建新用户，未携带或非管理员应被拒绝并写入审计。  
4) 强制改密：使用默认口令登录后被引导到改密页，设置符合策略的新密码；改密后旧口令不可再登录，旧令牌访问将被 403 并产生审计。  
5) 重新登录：用新密码登录成功，注册接口可用，权限正常。  
6) 负例：使用长度<8或缺少特殊字符的密码注册/改密，应被前后端共同拒绝并提示校验错误。
