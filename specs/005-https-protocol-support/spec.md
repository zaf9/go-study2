...
4. **Given** 配置项 `https.enabled = true` 且提供自签名证书, **When** 启动服务并由客户端发起请求, **Then** 服务可成功完成 TLS 握手（支持自签名证书信任策略）
...
- **CLI 学习模式兼容性**：开启或关闭 HTTPS 不得破坏 CLI 学习模式（若 CLI 不使用网络，则需在文档中声明无影响）
- **客户端校验证书被跳过的测试模式**：允许在测试/开发环境通过可配置项跳过客户端证书校验，需有风险提示
...
- **FR-010**: 当启用 HTTPS 时，HTTP 端口必须被禁用或拒绝明文访问
- **FR-011**: HTTPS 支持不得破坏 CLI 学习模式；若 CLI 无网络依赖，需明确声明且保持行为不变
...
- **SC-004**: 启用 HTTPS 后，HTTP 明文端口被禁用或拒绝连接，且有清晰日志
- **SC-005**: 自签名证书场景可成功完成 TLS 握手（在受控的测试/开发环境），支持可配置的客户端信任策略
- **SC-006**: 切换协议所需步骤（修改配置并重启）在自动化脚本或集成测试中测得的耗时 ≤ 30 秒
- **SC-007**: CLI 学习模式在 HTTP/HTTPS 配置切换后仍可正常使用（如无网络依赖则需文档确认）
...
- Q: 自签名证书如何处理？ → A: 支持自签名证书，提供可选 root CA 配置或（仅限测试/开发）跳过客户端校验的开关，并在文档中明确风险
- Q: CLI 学习模式是否受影响？ → A: 若 CLI 无网络依赖，则无需变更，但需记录兼容性说明和验证步骤
...
- 测试/开发环境可通过配置项跳过客户端证书校验，生产环境默认启用校验或由客户端控制信任
- CLI 学习模式的行为保持不变，需在验证步骤中确认
