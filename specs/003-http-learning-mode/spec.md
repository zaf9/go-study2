...
1. **Given** 用户需要在特定端口运行服务，**When** 修改config.yaml中port为9090并使用 `-d` 启动程序，**Then** HTTP服务在9090端口监听
2. **Given** 用户需要绑定特定IP地址,**When** 修改config.yaml中host为"127.0.0.1"并使用 `-d` 启动程序,**Then** HTTP服务仅在本地回环地址监听
...
- **内容生成失败**: 如果某个章节的Get*Content()函数出错或返回空内容，两种模式应如何一致地处理这种错误情况？
...
- **内容生成器 (ContentGenerator)**: 重构后的章节Display函数，返回字符串内容而非直接打印（命名为`Get*Content()`，如`GetCommentsContent()`），供命令行和HTTP模式共同使用
